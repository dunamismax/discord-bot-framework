// Package metrics provides performance monitoring and statistics collection for the MTG Card Bot.
package metrics
...
type Metrics struct {
	...
	ErrorsByType map[mtgErrors.ErrorType]int64
...
func Initialize() *Metrics {
	once.Do(func() {
		globalMetrics = &Metrics{
			ErrorsByType:  make(map[mtgErrors.ErrorType]int64),
...
func (m *Metrics) IncrementError(errorType mtgErrors.ErrorType) {
	m.mutex.Lock()
...
type Summary struct {
	...
	// Error statistics.
	ErrorsByType map[mtgErrors.ErrorType]int64 `json:"errors_by_type"`
